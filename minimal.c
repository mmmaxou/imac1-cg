...
void closePath(SDL_Event *e, PrimitiveList *p);
...
void closePath(SDL_Event *e, PrimitiveList *p) {
  if ( e->button.button == SDL_BUTTON_RIGHT 
      && (*p)->primitiveType == GL_LINE_STRIP ) {
    printf("click droit\n");
    (*p)->primitiveType = GL_LINE_LOOP;
    addPrimitive(allocPrimitive(GL_LINE_STRIP), p);
  }
}